...
- このポートフォリオでは、実際に実務を進める中で必要性を感じた「変更への強さ」や「継続的な品質保証」を意識し、`vitest`, `storybook`, `playwright`, `msw` などを使用し、己の学びとしてテスト・UI検証を少量ですが実装しています。
- 実際にこの実装を行うことで、テストを一通り組み上げる過程とともに、実務においてこれらのようなテストツールが何故広く使われているのかを肌で実感する良い経験となりました。
- 反面、テストコードを愚直に書きすぎると、アプリケーションの仕様変更時にテストの修正負荷が高まり、規模が大きくなるほど「メンテナンスコスト」が重くなりそうだという、実務的な課題にも気づく契機となりました。

あくまでこれらを使用した主たる動機としては知見を浅くでも広げるために触ってみようの精神で使ってみました。 \
実際に使用していく中で実務での使用を想像することで現在の実務に於いて足りていない部分などに気付かされました。
