...
type MainCardContentNode = {
  ...
  /**
   * カード本文として表示するJSX要素 \
   * `string`はisContentLabel=trueの場合に限定するため、 \
   * JSX.Elementを明示的に使用してstring型を排除しています
   */
 ...
 * - `false`（または未指定）: `description`に直接JSX（`JSX.Element`）を渡すことができます。
 *
...
const MainCard = (props: MainCardProps) => {
  const { image, description, actions, isDescriptionText, slotProps = {} } = props;
  const {
    card = {},
    cardActions = {},
    cardContent = {},
