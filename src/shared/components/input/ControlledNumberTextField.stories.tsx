...
import ControlledNumberTextField, {
  ControlledNumberTextFieldProps,
} from './ControlledNumberTextField.component';

// ----------------------------------------
// ダミースキーマの準備
// ----------------------------------------
type DummySchema = { age: number | null };

type FormWrapperProps = Omit<ControlledNumberTextFieldProps<DummySchema>, 'control' | 'name'>;
...
const FormWrapper = (props: FormWrapperProps) => {
  ...
  const dynamicSchema = useMemo(() => {
    const numberSchema = z
      ...
      .nonnegative({ error: '正の数で入力してください。' })
      .int({ error: '整数で入力してください。' })
      .nullable();

    return z.object({
      age: props.required
        ? numberSchema.refine((value) => value !== null, { error: '年齢は必須です。' })
        : numberSchema,
    });
  }, [props.required]);
    ...
    <DefaultForm method={method} onSubmit={onSubmit}>
      <FlatPaper>
        <Stack maxWidth={400} gap={2}>
          <ControlledNumberTextField {...props} control={method.control} name="age" />
 ...
 */
const meta = {
  title: 'Component/Input/ControlledNumberTextField',
